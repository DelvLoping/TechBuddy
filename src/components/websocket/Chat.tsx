...
const Chat = ({ isShow = true }: ChatProps) => {
  ...
  const handleSendMessage = (e) => {
    ...
    if (messageInput.trim() && selectedChat) {
      socket.emit('message', { chatId: selectedChat, userId: id, content: messageInput });
      setMessageInput('');
    }
  };

  const onBlur = () => {
    socket.emit('typing', { chatId: selectedChat, userId: id, isTyping: false });
  };
      ...
      {(isShow || show) && (
        <div className='flex flex-row items-center h-[35rem]'>
          ...
          <div className='flex flex-col rounded-r-xl border border-gray-200 w-[20rem] h-full bg-white'>
            ...
            <div className='flex-1 flex flex-col gap-2 py-4 px-4 overflow-y-auto bg-white'>
